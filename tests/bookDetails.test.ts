...
describe("showBookDtls", () => {
  ...
  it("should return 404 if the book instance is null", async () => {
    ...
    expect(res.send).toHaveBeenCalledWith(
      `Book details not found for book ${id}`
    );
  });

  it("should return 404 if the book is null", async () => {
    const id = "12345";
    // Mocking the Book model's findOne method to throw an error
    const mockFindBookInstance = jest.fn().mockReturnValue({
      select: jest.fn().mockReturnThis(), // Select is called here
      exec: jest.fn().mockResolvedValue(mockCopies),
    });
    BookInstance.find = mockFindBookInstance;

    const mockFindBook = jest.fn().mockReturnValue({
      populate: jest.fn().mockReturnThis(),
      exec: jest.fn().mockResolvedValue(null),
    });
    Book.findOne = mockFindBook;
    // Act
    await showBookDtls(res as Response, id);
  ...
  it("should return 404 if the id is invalid", async () => {
    ...
    // Act
    await showBookDtls(res as Response, id as any);
